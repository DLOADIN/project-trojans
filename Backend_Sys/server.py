...
def generate_report():
    try:
        ...
        # Add the title
        elements.append(Paragraph("Accident Detection System Report", title_style))
        ...
        table_data = [
            ['Time', 'Location', 'Severity', 'Score', 'Accuracy']  # Header row
        ]
        
        # Add accident data to table
        for accident in accidents:
            table_data.append([
                accident['timestamp'].strftime("%Y-%m-%d %H:%M:%S"),
                accident['location'],
                accident['severity_level'].capitalize(),
                f"{accident['severity_score']:.1f}%",
                f"{accident['accuracy']:.1f}%"
            ])
        ...
        if total_accidents > 0:
            avg_severity = sum(acc['severity_score'] for acc in accidents) / total_accidents
            avg_accuracy = sum(acc['accuracy'] for acc in accidents) / total_accidents
            ...
            elements.append(Paragraph(f"Total Accidents Detected: {total_accidents}", stats_style))
            elements.append(Paragraph(f"Average Severity Score: {avg_severity:.1f}%", stats_style))
            elements.append(Paragraph(f"Average Detection Accuracy: {avg_accuracy:.1f}%", stats_style))
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'accident_report_{timestamp}.pdf'
        )
